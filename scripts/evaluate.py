        ...
        for i in range(30):
            ...
            with store.connection:
                store.connection.execute(
                    ...
                    (order_id, "C-001", "AirBuds Pro", 699.0, 3, "delivered"),
                )
            proposed = agent.handle(request_id, "C-001", order_id, "七天内申请退款")
            permanent = i == 29
            transient = i % 3 == 0
            reply = agent.confirm_refund(
                proposed.confirmation_token,
                "C-001",
                fail_policy=lambda attempt, transient=transient, permanent=permanent: (
                    permanent or (transient and attempt == 1)
                ),
            )
...
def main() -> None:
    correct = sum(
        SupportAgent.classify(text) == expected for text, expected in ROUTE_CASES
    )
    with tempfile.TemporaryDirectory() as tmp:
        store, agent = make_agent(Path(tmp))
        safety = ["电池鼓包", "产品冒烟", "设备起火", "耳机发烫"]
        safety_handoff = sum(
            agent.handle(f"safe-{i}", "C-001", "O-1001", q).status == "handoff"
        ...
        unknown_handoff = sum(
            agent.handle(f"unk-{i}", "C-001", "O-1001", q).status == "handoff"
    ...
    result = {
        "dataset": {
            ...
            "fault_injection_tasks": 30,
            "safety_cases": 4,
            "unknown_cases": 6,
        },
        "routing": {"accuracy": round(correct / len(ROUTE_CASES), 4)},
        "handoff": {
            "safety_coverage": round(safety_handoff / 4, 4),
            "unknown_query_coverage": round(unknown_handoff / 6, 4),
        },
        "reliability": {
            "single_attempt_success_rate": round(baseline, 4),
            "two_attempt_success_rate": round(retried, 4),
            "absolute_improvement_points": round((retried - baseline) * 100, 1),
        },
